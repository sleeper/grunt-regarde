...
Regarde.prototype.add = function add(name, pattern, tasks, spawn) {
  var _this = this;
  var watcher;
  console.log('FRED: in add');
  this.logcb("Watching " + pattern);
  watcher = new this.watcherClass(pattern, function (err) {
    console.log('FRED: cb');
    if (err) {
